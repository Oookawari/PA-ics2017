...
static inline void rtl_sext(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  ...
  if(width == 1){
    *dest = *src1 & 0x0000007F;
    unsigned int temp = (*src1 & 0x00000080);
    (temp == 0 ? (*dest = *dest | (0x00000000)) : (*dest = *dest | (0xFFFFFF80)));
...
static inline void rtl_msb(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  ...
  if(width == 1){
    ...
    unsigned int temp = (*src1 & 0x80000000);
    has_sign = (temp == 0 ? 0 : 1);
...
static inline void rtl_update_SF(const rtlreg_t* result, int width) {
  ...
  assert(width == 1 || width == 2 || width == 4);
  if(width == 1){
    unsigned int temp = (*result & 0x00000080);
    ...
    unsigned int temp = (*result & 0x00008000);
    bool has_sign = (temp == 0 ? 0 : 1);  
    if(has_sign) {
      rtl_li(&t1, 1);
      rtl_set_SF(&t1);
